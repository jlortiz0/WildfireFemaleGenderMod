...
public class WildfirePronounScreen extends BaseWildfireScreen {
	...
	public void init() {
		...
		textFieldWidget = new TextFieldWidget(this.textRenderer, this.width / 2 - 100, y, 200, 20, Text.translatable("wildfire_gender.pronoun_list.title"));
		...
		this.addDrawableChild(new WildfireButton(this.width / 2 - 100, y + 110, 200, 20, Text.translatable("wildfire_gender.acknowledge.cancel"),
				button -> MinecraftClient.getInstance().setScreen(parent)));
		this.addDrawableChild(new WildfireButton(this.width / 2 - 100, y + 84, 200, 20, Text.translatable("wildfire_gender.acknowledge.confirm"),
				button -> {
	...
	public void render(DrawContext ctx, int f1, int f2, float f3) {
		...
		this.textFieldWidget.render(ctx, f1, f2, f3);
		ctx.fill(x - 161, y, x + 162, y + 48, 0x55000000);
		super.render(ctx, f1, f2, f3);

		ctx.drawText(this.textRenderer, Text.translatable("wildfire_gender.pronouns_list.title"), x - 40, 20, 16777215, false);
	}
}