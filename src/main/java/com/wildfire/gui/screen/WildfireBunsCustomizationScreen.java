...
public class WildfireBunsCustomizationScreen extends BaseWildfireScreen {
    ...
    public WildfireBunsCustomizationScreen(Screen parent, UUID uuid) {
        super(Text.translatable("wildfire_gender.buns_settings.title"), parent, uuid);
    }
    ...
    public void init() {
        ...
        this.addDrawableChild(this.bunsSlider = new WildfireSlider(this.width / 2 + 30, j - 48, 158, 20, Configuration.BUNS_SIZE, plr.getBunsSize(),
              plr::updateBunsSize, value -> Text.translatable("wildfire_gender.wardrobe.slider.buns_size", Math.round(value * 100)), onSave));
        ...
        this.addDrawableChild(this.xOffsetBunSlider = new WildfireSlider(this.width / 2 + 30, j - 27, 158, 20, Configuration.BUNS_OFFSET_X, buns.getXOffset(),
              buns::updateXOffset, value -> Text.translatable("wildfire_gender.wardrobe.slider.separation", Math.round((Math.round(value * 100f) / 100f) * 10)), onSave));
        this.addDrawableChild(this.yOffsetBunSlider = new WildfireSlider(this.width / 2 + 30, j - 6, 158, 20, Configuration.BUNS_OFFSET_Y, buns.getYOffset(),
              buns::updateYOffset, value -> Text.translatable("wildfire_gender.wardrobe.slider.height", Math.round((Math.round(value * 100f) / 100f) * 10)), onSave));
        this.addDrawableChild(this.zOffsetBunSlider = new WildfireSlider(this.width / 2 + 30, j + 15, 158, 20, Configuration.BUNS_OFFSET_Z, buns.getZOffset(),
              buns::updateZOffset, value -> Text.translatable("wildfire_gender.wardrobe.slider.depth", Math.round((Math.round(value * 100f) / 100f) * 10)), onSave));

        this.addDrawableChild(this.gapSlider = new WildfireSlider(this.width / 2 + 30, j + 36, 158, 20, Configuration.BUNS_GAP, buns.getGap(),
              buns::updateGap, value -> Text.translatable("wildfire_gender.wardrobe.slider.rotation", Math.round((Math.round(value * 100f) / 100f) * 100)), onSave));
        this.addDrawableChild(this.rotSlider = new WildfireSlider(this.width / 2 + 30, j + 57, 158, 20, Configuration.BUNS_ROT, buns.getRot(),
                buns::updateRot, value -> Text.translatable("wildfire_gender.wardrobe.slider.rotation_vert", Math.round((Math.round(value * 100f) / 100f) * 100)), onSave));

        this.addDrawableChild(new WildfireButton(this.width / 2 + 30, j + 78, 158, 20,
              Text.translatable("wildfire_gender.breast_customization.dual_physics", Text.translatable(buns.isUnibun() ? "wildfire_gender.label.no" : "wildfire_gender.label.yes")), button -> {
            boolean isUnibun = !buns.isUnibun();
            if (buns.updateUnibun(isUnibun)) {
                button.setMessage(Text.translatable("wildfire_gender.breast_customization.dual_physics", Text.translatable(isUnibun ? "wildfire_gender.label.no" : "wildfire_gender.label.yes")));
                GenderPlayer.saveGenderInfo(plr);
            }
        }));
