...
public class WildfireCharacterSettingsScreen extends BaseWildfireScreen {
    ...
    public void render(MatrixStack m, int f1, int f2, float f3) {
        ...
        if(plrEntity != null) {
            drawCenteredText(m, this.textRenderer, plrEntity.getDisplayName(), x, yPos - 30, 0xFFFFFF);
        }

        if(bounceWarning) {
            drawCenteredText(m, this.textRenderer, Text.translatable("wildfire_gender.tooltip.bounce_warning").formatted(Formatting.ITALIC), x, y+90, 0xFF6666);
        }
    }
