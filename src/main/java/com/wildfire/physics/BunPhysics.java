	...
	public void update(PlayerEntity plr, IGenderArmor armor) {
		...
		if(plr.getVehicle() != null) {
			...
			if(plr.getVehicle() instanceof MinecartEntity) {
				float speed = (float) ((MinecartEntity) plr.getVehicle()).getVelocity().lengthSquared();
		...
		if(plr.handSwinging && plr.age % 5 == 0 && plr.getPose() != EntityPose.SLEEPING) {
			if(Math.random() > 0.5) {
				targetBounce += -0.25f * bounceIntensity;
			} else {
				targetBounce += 0.25f * bounceIntensity;
			}
		}
